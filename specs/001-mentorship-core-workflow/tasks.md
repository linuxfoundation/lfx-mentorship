...
- [x] T026 [US3] Add `UserProfileRepository.CountActiveMenteeProfiles(ctx, userID string) (int, error)` to repository interface and implement in `backend/internal/infrastructure/db/user_profile_repository.go` (`SELECT COUNT(*) … WHERE user_id=$1 AND profile_type='mentee'`)
- [x] T027 [US3] Add eligibility gate to `UserProfileService.Create` in `backend/internal/service/user_profile_service.go`: when `profile_type = mentee`, call `CountActiveMenteeProfiles`; if > 0, return `ErrIneligible` with message `"user already has an active mentee profile"` (age and work-eligibility criteria are client-side confirmation gates passed in as booleans on the create input)
- [x] T028 [P] [US3] Add `AgeEligible` and `WorkEligible` boolean fields to `UserProfileCreateInput` in `backend/internal/domain/models/user_profile.go`; validate both must be `true` in `UserProfileService.Create` when `profile_type = mentee`
