  ...
  >
    <template #message="slotProps">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p
            v-if="toastHeading(slotProps.message)"
...
const toastTypeValues = new Set<string>(Object.values(ToastTypesEnum));

const toastHeading = (message: ToastOptions) => {
  if (message.title) return message.title;
  const summary = message.summary;
  if (!summary || toastTypeValues.has(String(summary))) return undefined;
  return summary;
};
