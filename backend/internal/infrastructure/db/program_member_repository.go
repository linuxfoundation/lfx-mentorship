...
func (r *ProgramMemberRepository) ListAdminsByProgram(ctx context.Context, programID string) ([]*models.ProgramAdmin, error) {
	ctx, span := programMemberTracer.Start(ctx, "db.program_admins.ListByProgram")
	defer span.End()

	rows, err := r.pool.Query(ctx,
		`SELECT id, program_id, user_profile_id, role, created_on, updated_on
		 FROM program_admins WHERE program_id = $1 ORDER BY created_on`,
		programID,
	)
	if err != nil {
		span.RecordError(err)
	...
	var admins []*models.ProgramAdmin
	for rows.Next() {
		var a models.ProgramAdmin
		if err := rows.Scan(&a.ID, &a.ProgramID, &a.UserProfileID, &a.Role, &a.CreatedOn, &a.UpdatedOn); err != nil {
			span.RecordError(err)
	...
	if admins == nil {
		admins = []*models.ProgramAdmin{}
	}
	return admins, rows.Err()
}

// AddAdmin links a user profile to a program as admin.
func (r *ProgramMemberRepository) AddAdmin(ctx context.Context, programID string, input models.ProgramAdminCreateInput) (*models.ProgramAdmin, error) {
	ctx, span := programMemberTracer.Start(ctx, "db.program_admins.Add")
	defer span.End()

	var a models.ProgramAdmin
	err := r.pool.QueryRow(ctx,
		`INSERT INTO program_admins (program_id, user_profile_id, role)
		 VALUES ($1, $2, $3)
		 ON CONFLICT (program_id, user_profile_id) DO UPDATE SET role = EXCLUDED.role
		 RETURNING id, program_id, user_profile_id, role, created_on, updated_on`,
		programID, input.UserProfileID, input.Role,
	).Scan(&a.ID, &a.ProgramID, &a.UserProfileID, &a.Role, &a.CreatedOn, &a.UpdatedOn)
	if err != nil {
		span.RecordError(err)
...
func (r *ProgramMemberRepository) DeleteAdmin(ctx context.Context, adminID string) error {
	ctx, span := programMemberTracer.Start(ctx, "db.program_admins.Delete")
	defer span.End()

	cmd, err := r.pool.Exec(ctx, `DELETE FROM program_admins WHERE id = $1`, adminID)
	if err != nil {
		span.RecordError(err)
