...
func (s *relayOutboxStub) Retry(_ context.Context, marker domain.FGAOutboxMarker, nextAttemptAt time.Time, _ string) error {
	s.retried = append(s.retried, marker.ID)
	s.nextAttemptAt = nextAttemptAt
	return s.retryErr
}
func (s *relayOutboxStub) DeadLetter(_ context.Context, marker domain.FGAOutboxMarker, _ string) error {
