...
| GW-4 | Timing: does the Heimdall cutover gate the public launch, or does Mentorship launch on the interim model (as deployed by [lfx-mentorship#148](https://github.com/linuxfoundation/lfx-mentorship/pull/148), still open, and [lfx-v2-argocd#1453](https://github.com/linuxfoundation/lfx-v2-argocd/pull/1453), merged) and cut over after? | Launch interim, cut over after — the interim model is exactly what Crowdfunding runs in production today, and blocking launch on cross-repo PRs buys no user-visible safety while nothing is live to migrate. **This is conditional, not unqualified.** "Cut over after" is safe only for the routes whose worst case is a *missing* restriction on non-sensitive data; it is not safe where the interim model actively serves PII to anonymous callers. **GW-8's gating and redaction are therefore a pre-launch prerequisite of this answer, not cutover work** — the same standing GW-9's two remaining gaps already have. Launching interim while `GET /v1/user-profiles` returns phone, address, demographics and socioeconomics to unauthenticated callers would not be "the model Crowdfunding runs" — Crowdfunding has no equivalent surface. Sequence: close GW-8 and GW-9 in the service handlers, launch, then cut over. |
