...
**Step 6 also depends on a prerequisite outside this repo's five PRs.** [lfx-v2-project-service](https://github.com/linuxfoundation/lfx-v2-project-service) owning and emitting `mentorship_program_admin` (line 108 above; PR 5 of [04 §implementation path](./04-authorization-model.md)) is "not optional" per that document — without it, `mentorship_program.writer` inherits through a relation with no durable tuples, and every cross-program admin check fails closed (AQ-4). That work is not one of the seven cutover steps and is not represented in the flowchart above because it ships in a different repo on its own timeline; it must still be rolled out and its tuples verified in FGA before step 6 runs in an environment, or cross-program admins silently lose access the moment `heimdall.enabled: true` takes effect. Treat it as a prerequisite check alongside step 4's coverage verification, not merely as a parallel dependency to track.
...
| GW-2 | Public catalog reads: `viewer@user:*` wildcard check everywhere, or `allow_all` plus the service's published filter for the collection routes? | **Resolved: split by route shape.** `GET /v1/programs/catalog` is a collection with no object UID to put in an `openfga_check`, so the wildcard cannot express it; that route gets `allow_all` and keeps the service's existing `status = published` filter. ID-addressed public reads keep the wildcard check. Note the distinction the RuleSets must preserve: a genuinely public route uses `allow_all` **unguarded**, as `lfx-v2-newsletter-service` does for its tracking pixel (`rule:lfx:lfx-v2-newsletter-service:newsletter_opens:pixel`). The `allow_all` that appears in the `{{- else }}` branch of an `{{- if .Values.openfga.enabled }}` guard is a *local-development* fallback, not a public route — meeting-service's chart says so explicitly. Writing a public route as the guarded form would silently drop the check the moment OpenFGA is enabled. **`GET /v1/programs` (`server.go:133`) is the same collection shape and needs the same treatment, but is not yet pinned:** `ProgramHandler.List` forwards `?status=` straight through, and unlike `ListCatalog` (which pins `Status = published` in `normalizeCatalogFilter`), `ProgramService.List` and `ProgramRepository.List` apply the caller-supplied value verbatim, defaulting to `published` only when the parameter is absent — so an anonymous caller can pass `?status=draft`, `hidden`, `rejected`, or `submitted` today. `allow_all` on this route authenticates nothing about the object, so it cannot substitute for pinning the filter; the service must reject or ignore a non-public `status` value on this route the same way `GET /v1/programs/{id}/members` now pins `member_type` (GW-9), before this route can be treated as resolved under GW-2. |
