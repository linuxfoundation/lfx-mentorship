...
2. **Backfill.** One-shot, idempotent ETL: DynamoDB export → transform (un-nest program terms, split applications/enrollments, map LFIDs) → Postgres. Field-level mapping dictionary as in Crowdfunding's `data-design_and_migration.md`. Re-runnable; validated with row-count and spot-check reports per entity.
...
5. **Decommission.** After a stability window: remove legacy Lambdas, DynamoDB tables, Elasticsearch indexes/cluster, SNS/SQS wiring, and the Fivetran DynamoDB connector. Employers table archived (export to S3) before deletion.
