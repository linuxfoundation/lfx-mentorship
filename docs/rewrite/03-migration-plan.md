...
flowchart LR
    ...
    P3 --> P4["4 · Cutover<br/>content freeze, final backfill,<br/>DNS switch"]
...
2. **Backfill.** One-shot, idempotent ETL: DynamoDB export → transform (un-nest program terms, split applications/enrollments, map LFIDs) → Postgres. Field-level mapping dictionary as in Crowdfunding's `data-design_and_migration.md`. Re-runnable; validated with per-entity row-count reports, field-level source-to-target reconciliation (every source field maps to the expected target column), and referential-integrity checks (every un-nested child row resolves to the correct parent) — counts and spot checks alone are not sufficient for a document-to-relational transformation.
...
4. **Cutover.** Short content freeze on prod writes → final backfill → point `mentorship.lfx.linuxfoundation.org` at the new frontend → verify journeys. Legacy stack kept read-only.
...
**Rollback:** the legacy stack remains intact until decommission, but rollback is only clean in the immediate post-cutover window, **before the new stack accumulates writes**: a DNS revert plus re-enabling legacy writes loses nothing only while Postgres has no data DynamoDB lacks. Once new writes land (applications, task updates, program changes), rolling back requires either a reverse sync (Postgres → DynamoDB) or accepting the loss of those writes. The plan therefore treats rollback as a break-glass option for the first hours after cutover — the post-cutover smoke-test window is sized so a go/no-go decision is made before meaningful write volume accrues. No reverse-sync tooling is built; past that window, the remedy is fix-forward.
