...
1. **Public program-page visibility is asymmetric by role**, matching the legacy public page exactly:
   ...
   - **Mentees — accepted and graduated.** The legacy page renders them as two distinct sections fed by two separate calls: "Mentees" from `getProjectMentees` and "Graduated Mentees" from `getAllProjectMenteesByStatus(..., 'graduated', ...)`, the latter paginated (`project-public.component.ts:56-59`, `project-public.component.html:38-113`). Graduated mentees stay listed after their term ends — the program's alumni record. Keep both sections.
