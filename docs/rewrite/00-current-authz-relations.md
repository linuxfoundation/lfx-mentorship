...
1. **Public program-page visibility is asymmetric by role.** Program publication status is the *only* gate — never term status, application-window status, or any other time-varying condition:
   ...
   - **Deliberate divergence — drop the time-varying gate entirely.** Legacy hides *both* mentee sections unless the program **either** has an open application window **or** an active term (`showAcceptedMentees`, computed from `areApplicationsOpenOrHasActiveTerms()`, `project-public.component.ts:62`). The rewrite drops both halves of that disjunction: accepted and graduated mentees are listed on every **published** program, regardless of the status of its terms or its application windows.

     This is the one visibility rule the rewrite intentionally changes, and it is a simplification in three ways. It removes a derived condition — no walking a program's terms and comparing window dates against the clock just to decide whether to render a roster. It makes the answer stable — the same program shows the same roster today and next month, so the page is cacheable and cannot silently empty itself as a date passes. And it collapses the public-visibility rule to a single predicate, `program.status = published`, that is uniform across mentors and mentees rather than one gate for one role and a compound gate for the other.

     Nothing is protected by the legacy gate: the participants it hides were public the day before the window closed, and it erases a program's alumni record exactly between terms, when the roster is most useful to a prospective applicant. Unpublished programs still expose nothing.
