...
1. **Public mentor visibility**: the public program page shows only **accepted** mentors. Pending, invited-but-not-accepted, declined, and withdrawn mentors are never publicly listed (an invitee has not consented to a public affiliation). The review asked for "accepted and graduated", but graduation is a mentee-only concept in legacy — the status constant is documented as *"Mentee has graduated the Mentorship program"* (`project.model.ts:348`) and the mentor validator rejects it (`project.go:1181`). Adding a mentor `graduated` state would be new behavior, not replication, so the rule resolves to "accepted only"; mentors stay listed for as long as they hold the accepted relation. Revisit only if mentor graduation becomes a product requirement in its own right.
...
5. **Member emails are never public**: the public program surface is program metadata, accepted mentors (name/avatar/bio/public profile link), and — when the program opts in — accepted mentees (name/avatar/bio). No emails, for any member type. This reproduces the product exactly and closes divergence 8; the anonymous member-list API is a defect, not a feature.
