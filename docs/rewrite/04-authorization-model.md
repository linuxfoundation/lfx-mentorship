...
**Statuses**: mentee applications run `pending → accepted → graduated`, or `declined` / `withdrawn`. `accepted` is both the admission decision and the enrolled state — a mentee stays `accepted` for the term and graduates from it. There is no `active` application status (AQ-10, resolved).
...
**No enrollment entity.** In the legacy system the application *is* the lifecycle object: one `program-term-mentees` row (keyed by user + program term — this is the term-keyed mentee source per [00](./00-current-authz-relations.md); the program-level `project-members` table, whose `apprentice` memberType names the same role, carries no term identity) whose status runs the full journey `pending → accepted → graduated`. Acceptance and graduation are status changes on that row, and mentors relate to the **program**, not to individual mentees (a mentee's "mentors" list is a cron-denormalized copy of the program's approved mentors). The rewrite keeps that shape — no `enrollments` table, no mentor-mentee assignment — and the ERD in [02](./02-target-architecture.md) reflects it.
...
**Who appears on a public program page is a business rule, not a relation.** `viewer` authorizes access to *the page*; the roster it returns is a Postgres query — **`active`** mentors, and accepted **plus graduated** mentees ([00](./00-current-authz-relations.md), decision 1). No emails, for any member type. Note the vocabulary split: mentors are `program_members` rows and their statuses are `invited | requested | pending | active | declined | withdrawn` (`001_initial.up.sql:161`), so the public mentor filter is `status = 'active'` — there is no `accepted` member status, and naming one would fail the CHECK constraint. Mentees are `applications` rows, where `accepted` and `graduated` do exist — and since AQ-10 removed the `active` application status, `accepted` is also the enrolled state, so the mentee predicate is `status IN ('accepted', 'graduated')`. Both filters are now applied: `ListCatalogMentors` (`program_repository.go:483`) and, since [lfx-mentorship#144](https://github.com/linuxfoundation/lfx-mentorship/pull/144), `GET /v1/programs/{id}/members` — see [05 GW-9](./05-heimdall-gateway.md). Because that roster is gated solely on `program.status = published` — never on term status, application-window state, or any other time-varying condition — the wildcard tuple is a *complete* authorization for the public surface: nothing is left for the service to re-check after the edge has allowed the request. This is a deliberate divergence from legacy, which hid its mentee sections unless the program currently had an open application window or an active term. That gate could not have been expressed as a tuple at all, so reproducing it would have meant the edge check was never the whole answer. Implementers should not restore it from the legacy frontend.
...
The table maps against the **post-review revision** of the standalone model, which folded in three structural changes agreed on the call: `super_admin` dropped as redundant with project `writer`; approval moved to a standalone global approver type; and `program`/`application`/`task` renamed with the `mentorship_` prefix. Two corrections this doc had raised against the earlier draft — mentors holding `can_create_term`, and a single `can_update_status` spanning submission and review — are resolved in that revision and are recorded below as settled.
...
| `project.mentorship_program_admin`; `can_create_program` | same relation name; `mentorship_program_creator`; `mentorship_program_admin from project` in the program's `writer` | `mentorship_program_creator` is a pure computed union — no tuples of its own. The revision has no separate "manage every program" relation on `project`: cross-program management is expressed by projecting the relation *into* the program (`define project_admin: mentorship_program_admin from project`), which folds into `admin` there. Same resolved set as this sketch's `mentorship_program_admin from project` in `writer` |
| program `admin`; `can_edit`, `can_submit`, `can_invite_mentor` | `writer` | |
| program `can_create_term` (`project_admin`) | `writer` | **mentor exclusion settled; scope still open.** The earlier draft granted term creation to mentors (asserted in `tests.yaml`); this doc raised it as a product mismatch and the revision dropped it — corroborated by the legacy baseline, where [00](./00-current-authz-relations.md) records term management enforced at "any membership" (mentor included) against an admin-only product intent, the same backend-looser-than-intent pattern as mentee-application approval (divergence 2). What remains is a **narrower** scope than this sketch: the revision resolves term creation to the project-wide `mentorship_program_admin` pool only, so a directly-assigned per-program admin cannot open a term for the program they run. This sketch's `writer` admits all three (direct, project writer, cross-program admin). Raised with the model's author; unresolved |
...
| task `admin`/`mentor` from application; `can_update_status` (assignee or admin or mentor) | `manager: reviewer from mentorship_application`; `auditor: assignee or manager`; submission checks `assignee`, review checks `manager` | **settled.** The earlier draft carried a single `can_update_status` on the *application*, spanning both the applicant's own status changes and admin review; this doc raised that one relation cannot gate the two routes differently, as decision 6 requires. The revision splits them — `can_decide` (admin-only) on `mentorship_application`, `can_update_status` on `mentorship_task` — which is the same decomposition, at the same boundary. Task-level `can_update_status` remains a single union over assignee and reviewers; this sketch keeps them as separate relations so submission and review stay separately checkable |
...
6. **No attribute-level access control → split the endpoints.** Heimdall authorizes a *route*, not a field: per the Architecture call, *"if you want to have different attributes requiring different relationship checks, you'll want to actually split those across two different REST endpoints."* Mentorship hits this immediately, because mentors may evaluate an application but not change its status. So there is no single `PATCH /applications/{uid}` accepting an arbitrary field set. Instead:
   ...
   **Redaction and route-splitting are not interchangeable, and the boundary is *who is asking*.** Split the route when what may be seen depends on the caller's relation — the reviewer note, where the applicant holds `auditor` and must still be excluded, so a conditionally-populated field would be the service making an authorization decision. Redact in the handler when a field is withheld from *every* caller the route admits, regardless of relation — the member-list email ([05](./05-heimdall-gateway.md), GW-9, already fixed in [lfx-mentorship#144](https://github.com/linuxfoundation/lfx-mentorship/pull/144)), which no public caller may see and which no relation would distinguish, and where the column must stay on the model because create and update both accept it. The first is an authorization decision the service must not make; the second is a payload decision no relation could express. Applying the wrong one costs either a leak (redacting what should have been split) or a route per field (splitting what should have been redacted).
...
| AQ-10 | ~~**Does the `active` application status earn its place?**~~ **Resolved — dropped.** It had **no legacy equivalent** — jobspring's `ProjectMemberStatus` is `pending`, `accepted`, `rejected`, `approved`, `hold`, `withdrawn`, `graduated`, `declined` (`backend/project/project.go:1232-1248`); every `active` in that codebase was a different concept (`ProgramTerm.Active` is `open\|closed\|deleted`, `ActiveUserStatus` is an account state). It was also never written: no code path assigned it, and every read already paired it with `accepted` (`IN ('accepted', 'active', …)`), so it was dead in the schema. | **Removed** in [lfx-mentorship#144](https://github.com/linuxfoundation/lfx-mentorship/pull/144): the constant, the `IsValid()` case, the `applications_status_check` value, the transition hop (now `accepted → graduated` directly, so `graduated` is not orphaned), and the six SQL read lists. Every changed query is behavior-preserving, since no row could hold the removed value. Two `active` spellings deliberately remain, both **display** vocabulary rather than stored status: `models.MenteeStatus` (the directory label for an enrolled mentee) and the `?status=active` filter, which maps onto stored `accepted` — this is the "show it only in the UI" resolution. Unrelated and untouched: `program_members.status = 'active'`, a different column and the only status a mentor ever holds. The ETL consequence stands — legacy has nothing to backfill from, so every historical in-progress mentee lands on `accepted`. Note this was the **second** status-vocabulary divergence found in review; `hold` (above) is the other, still open, and unlike `active` it *does* have legacy precedent. |
