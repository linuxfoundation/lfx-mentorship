...
| AQ-10 | **Does the `active` application status earn its place?** It has **no legacy equivalent** — jobspring's `ProjectMemberStatus` is `pending`, `accepted`, `rejected`, `approved`, `hold`, `withdrawn`, `graduated`, `declined` (`backend/project/project.go:1232-1248`); every `active` in that codebase is a different concept (`ProgramTerm.Active` is `open\|closed\|deleted`, `ActiveUserStatus` is an account state). The rewrite introduced it as the middle of `accepted → active → graduated` (`internal/service/application_service.go:62-68`), which makes it mandatory on the path to graduation, and the public catalog therefore has to list it or mentees would vanish from the program page for the whole duration of their mentorship. So the status and the catalog filter cannot be decided separately. | **Drop it**, unless someone names a route or permission that depends on it. Nothing distinguishes "accepted but not started" from "running" except the term window, which is already stored — "currently running" is derivable (`accepted` + term dates), not a state worth persisting. Dropping it means: remove the constant, the `IsValid()` case, the `applications_status_check` value, the transition-map hop, the catalog `IN` list, and the status line in this doc. Keeping it means the ETL has to invent it — legacy has nothing to backfill from, so every historical in-progress mentee lands on `accepted` regardless. Decide before the ETL contract in [03](./03-migration-plan.md) is finalized. Note this is the **second** status-vocabulary divergence found in review; `hold` (above) is the other, and unlike `active` it *does* have legacy precedent. |
