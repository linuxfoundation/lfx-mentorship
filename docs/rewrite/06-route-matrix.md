...
| `DELETE /v1/me/profile/logo`, `DELETE /v1/me/avatar`, `DELETE /v1/me/profile/resume` | required | — | `allow_all` | Self-scoped removal, same `principal` rule. Null the column and remove the object — for the resume, **all versions**, per [02 §deleting the bytes](./02-target-architecture.md#deleting-the-bytes). Without these there is no way to withdraw a resume, which a private PII class has to support. These are not the only paths that must remove bytes: replacement and parent deletion do too, and are specified in 02 rather than per-row here |
