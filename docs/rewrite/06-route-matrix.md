...
The reservation is **per field, not per column**. `profile_links` is `{resumeLink, linkedinProfileLink, githubProfileLink}` (`backend/db/migrations/001_initial.up.sql:69`); only `resumeLink` is a file reference, so a generic write carrying that one field is rejected and the other two pass through. Reject rather than silently strip — otherwise the caller gets a success response for a write that did not happen. **The reserved name moves with the column:** the resume locator is being lifted out of the JSONB blob into a dedicated `user_profiles.resume_url` ([linuxfoundation/lfx-self-serve#1539](https://github.com/linuxfoundation/lfx-self-serve/issues/1539)), so the generic endpoints must reject `resume_url` once it exists, and keep rejecting `resumeLink` until the backfill has drained it. Reserving only the old name would leave the new column writable by the generic route — the exact bypass this rule exists to prevent.
...
1. **The public classes keep their service `GET` routes.** The CDN supplements them, it does not replace them — when `CDN_URL_PREFIX` is unset (local dev) the service route is the only path to the bytes, and it stays reachable during a CDN outage as an explicitly addressed alternative, since stored URLs are not rewritten by flipping the prefix ([02 §object storage](./02-target-architecture.md#object-storage)). **No *private*-class download route may be `anonymous`**; that would route around the bucket split.
2. **A size cap and the content-type allowlist apply to every upload route.** Heimdall authorizes the caller, not the payload. **20 MB is the platform ceiling, not the per-class limit:** the image classes (program logos, profile logos, avatars) cap at **2 MB**, and only private files (resumes, task submissions) get the full 20 MB ([02 §object storage](./02-target-architecture.md#object-storage)). The per-row notes name only what else differs by class.
3. **SVG is excluded from every image allowlist** — S3 serves it as executable content, making it a stored-XSS vector on any origin that renders it. This is a deliberate tightening, not the platform default: member-service admits SVG behind a purpose-built sanitizer, and Mentorship has none, so exclusion is the only safe posture here. It is also why the migration quarantines rather than drops legacy `image/svg+xml` logos ([03 §object storage](./03-migration-plan.md#migration-specific-tasks)).
