...
| `PUT /v1/tasks/{uid}/submission/file` | required | `mentorship_task:{uid}` | `assignee` | **Private class.** The mentee uploads; only before review closes, the same state rule as the `DELETE` below. Pairs with `PATCH /v1/tasks/{uid}/submission` in the task section |
...
Second, the 20 MB cap and the content-type allowlist are service concerns on **every** upload route — Heimdall authorizes the caller, not the payload — so the per-row notes name only what differs by class (image allowlist with SVG excluded for the public classes, PDF/doc for resumes).
...
| RM-5 | A user may hold both a mentor and a mentee profile row, so `PUT /v1/me/profile/logo` and `/resume` do not name a single row. Which profile is canonical for each file class? | **Address the profile explicitly** — `PUT /v1/user-profiles/{uid}/logo` with a self-ownership check — rather than inventing a precedence rule under `/me/`. The resume's three routes take the same form (`PUT`, `GET`, `DELETE /v1/user-profiles/{uid}/resume`), for the same reason and with the same self-ownership check; the `/me/profile/...` spellings in the table above are shorthand for them. `/me/avatar` stays as-is, since avatars live on `users`. A "most recently updated profile" tiebreak would silently write to the wrong row |
...
| RM-7 | The file routes here are spelled `PUT /v1/me/profile/logo` and `PUT /v1/tasks/{uid}/submission/file`; the platform singleton baseline in [lfx-object-store-design](https://github.com/linuxfoundation/lfx-skills/blob/main/skills/lfx-object-store-design/SKILL.md) is `POST /resources/{uid}/logo-upload`, `GET /resources/{uid}/logo-download`, `DELETE /resources/{uid}/logo`. Adopt the baseline spelling, or keep the resource-nested one? | **Open.** The divergence is real and this table does not currently admit it. The baseline's `-upload`/`-download` suffixes exist because it assumes no nesting under a parent resource; these routes are already nested and relation-guarded per row, so the suffixes add nothing here. Against that, a Mentorship-only spelling costs the shared SDK and any future generic client. Deciding it means renaming every file route in this table, which is an architecture call rather than a documentation fix |
