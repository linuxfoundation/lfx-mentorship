...
- **Public files** store the **full CDN URL**, cache-busting parameter included — `{CDN_URL_PREFIX}/{key}?v={upload-unix-timestamp}` — written once at upload and returned verbatim as `public_url`. This follows [lfx-v2-member-service](https://github.com/linuxfoundation/lfx-v2-member-service), the one V2 service that has shipped this pattern (`internal/service/logo_uploader.go`), and it resolves what would otherwise be a contradiction: the `?v=` token has to be persisted rather than refetched from S3 per response, but `avatar_url`/`logo_url` are single `TEXT` columns with nowhere to put a companion timestamp, and [03 §migration](./03-migration-plan.md#s3-objects) adds no schema change. Embedding the token in the stored URL is what makes both true at once.

  **Three pieces carry this, and all three are required together.** First, the **object key is deterministic** — one key per resource and file class (`program_logos/{program_uid}`, `avatars/{user_uid}`), reused by every upload, with no UUID and no file extension; `Content-Type` rides on the object rather than being inferred from the key. Second, the stored string is the versioned URL. Third, `Cache-Control: public, max-age=86400` is set as object metadata at upload. The determinism is the load-bearing part: because every upload overwrites the same key, a copy of the URL persisted anywhere else — a stale index row, a cached API response — points at a key whose *current* bytes are correct, so it converges within the TTL instead of serving superseded content indefinitely. A UUID-per-upload key would freeze every stale URL permanently. **Do not lengthen that TTL without revisiting this contract**, and do not use the S3 `VersionId` as the token — it is an addressing mechanism, and code that ever passes it to `GetObject` pins stale URLs to frozen bytes, defeating the convergence this depends on. The `?v=` parameter must be in the CloudFront cache key.

  The cost is explicit: `CDN_URL_PREFIX` is baked into a data column, so changing the CDN hostname requires a rewrite pass over every public-class row. That is accepted as the cheaper side of the trade against a schema change, and it is bounded — the deterministic key means the rewrite is a prefix substitution, not a re-derivation. The service's own download route does not parse the key back out of the stored URL; it composes the key from the record's own identity, which is exactly what determinism buys, so the route still works when `CDN_URL_PREFIX` is unset. `CopyObject` preserves source metadata by default, so the migration copy must pass `MetadataDirective: REPLACE` to set `ContentType` and `Cache-Control` rather than inheriting whatever the legacy bucket happened to store.
...
**Every path that drops a file's locator must also remove the object. Bucket lifecycle is not a substitute.** The ops lifecycle rules expire *noncurrent* versions ([lfx-object-store-ops](https://github.com/linuxfoundation/lfx-skills/blob/main/skills/lfx-object-store-ops/SKILL.md)); a current object whose last database reference has gone is unreferenced, not noncurrent, and no rule ever reaps it. For a resume or a task submission that means live PII retrievable by anyone holding the key, with no expiry — an erasure gap, not just a storage-cost one. Three paths need it, and only the first is covered by the dedicated file `DELETE` routes in [06](./06-route-matrix.md):

- **Replacement.** Public classes are safe by construction: the deterministic key means a re-upload overwrites in place, leaving no orphan (the prior payload becomes a noncurrent version, which lifecycle does expire). Private classes are **not** — `resumes/{user_uid}/{uuid}.pdf` mints a new key per upload, so a `PUT` over an existing resume strands the old object. Either delete the previous key as part of the upload, or key private singletons deterministically too. Deterministic keying is preferred, for the same reason it is used above: it makes the guarantee structural rather than dependent on a cleanup call that can fail after the row has already been rewritten.
- **Parent deletion.** `DELETE /v1/programs/{uid}`, `DELETE /v1/tasks/{uid}`, `DELETE /v1/me`, and `DELETE /v1/me/profile` all remove rows that hold file locators — `user_profiles.user_id` is `ON DELETE CASCADE`, so deleting a user erases `resumeLink` in Postgres without touching S3. Each must enumerate the file keys the deleted rows held and remove them.
- **Permanently, on a versioned bucket.** For private classes, removal means deleting **all versions and delete markers**, not a plain `DeleteObject` — which only adds a marker and leaves every prior payload retrievable by version. This is the same operation [03 §migration](./03-migration-plan.md#s3-objects) already specifies for the freeze-time reconcile; it needs to hold at runtime too, not only at cutover.

Deletion is best-effort *in ordering only*: it may lag the row delete, but it must be retried to completion rather than dropped, since a failure leaves PII behind with nothing pointing at it. A periodic reconcile — the freeze-time sweep in [03](./03-migration-plan.md#s3-objects), run on a schedule rather than once — is the backstop for keys that escape both paths.
