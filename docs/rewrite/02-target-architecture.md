...
- **Public files** store the **full CDN URL**, cache-busting parameter included — `{CDN_URL_PREFIX}/{key}?v={upload-unix-timestamp}` — written once at upload and returned verbatim as `public_url`. This follows [lfx-v2-member-service](https://github.com/linuxfoundation/lfx-v2-member-service), the one V2 service that has shipped the pattern (`internal/service/logo_uploader.go`). It resolves a contradiction: the `?v=` token must be persisted rather than refetched per response, but `avatar_url`/`logo_url` are single `TEXT` columns with nowhere for a companion timestamp, and [03 §migration](./03-migration-plan.md#s3-objects) adds no schema change. Embedding the token in the stored URL satisfies both.
  ...
  **Three pieces carry this, and all three are required together:** (1) a **deterministic object key** — one per resource and file class (`program_logos/{program_uid}`, `profile_logos/{profile_uid}`, `avatars/{user_uid}`), reused by every upload, no UUID and no extension, with `Content-Type` on the object rather than inferred from the key; (2) the stored string is the versioned URL; (3) `Cache-Control: public, max-age=86400` as object metadata at upload. **Determinism is the load-bearing part** — every upload overwrites the same key, so a URL persisted elsewhere (a stale index row, a cached response) points at a key whose *current* bytes are correct and converges within the TTL. A UUID-per-upload key would freeze every stale URL permanently. **Do not lengthen the TTL without revisiting this**, and do not use the S3 `VersionId` as the token: it is an addressing mechanism, and code that passes it to `GetObject` pins stale URLs to frozen bytes. The `?v=` parameter must be in the CloudFront cache key.
...
Deletion may lag the row delete, but must be retried to completion rather than dropped — a failure leaves PII behind with nothing pointing at it. The freeze-time sweep in [03](./03-migration-plan.md#s3-objects), run on a schedule, is the backstop for keys that escape both paths.
