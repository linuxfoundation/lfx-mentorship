...
**Implementers: read both skills before writing code or HCL.** This section states what Mentorship stores and where; it does not restate the platform baseline. [lfx-object-store-design](https://github.com/linuxfoundation/lfx-skills/blob/main/skills/lfx-object-store-design/SKILL.md) owns the application side — SDK wiring and the credential chain, `EnsureBucket`, the chart contract (`s3.endpointURL`, `s3.createMissingBucket`, `cdnURLPrefix`, the nats-s3 sidecar) — noting that Mentorship needs two buckets, so those values repeat per bucket under the skill's namespacing convention (`LOGOS_S3_BUCKET`/`ATTACHMENTS_S3_BUCKET`, `LOGOS_CDN_URL_PREFIX` on the public bucket only — **`CDN_URL_PREFIX` below is shorthand for `LOGOS_CDN_URL_PREFIX`**, the only public prefix Mentorship has), while process-wide settings such as `AWS_REGION` and the credential chain are not namespaced, and the local stack. [lfx-object-store-ops](https://github.com/linuxfoundation/lfx-skills/blob/main/skills/lfx-object-store-ops/SKILL.md) owns provisioning, in [lfx-v2-opentofu](https://github.com/linuxfoundation/lfx-v2-opentofu) rather than this repo. Where this document and a skill disagree, the skill is the baseline and the difference is a bug in this document — raise it rather than diverging silently. **One divergence is deliberate and named here.** The skill composes `public_url` per response from `CDN_URL_PREFIX` plus the key, and defines `CDN_URL_PREFIX=""` as a degraded mode that omits the field ([SKILL.md](https://github.com/linuxfoundation/lfx-skills/blob/main/skills/lfx-object-store-design/SKILL.md)); Mentorship instead **persists the composed URL in the column**, as member-service ships it. The reason is the `?v=` token below: it is a property of the object, not of the request, so a column holding only the key has nowhere to carry it and every read would have to refetch it from S3 — which [03 §object storage](./03-migration-plan.md#migration-specific-tasks) rules out. Degraded mode therefore resolves at write time, not read time: with no public prefix configured the public classes have no CDN, and the column holds the service download route path with the same `?v=` token appended. One column shape either way, and no schema change (tracked in [linuxfoundation/lfx-self-serve#1539](https://github.com/linuxfoundation/lfx-self-serve/issues/1539)).
...
Profile logos key on the **profile** UID, not the user UID: a user may hold both a mentor and a mentee profile row, which would otherwise overwrite each other.
...
- **Public files** store the **full CDN URL**, cache-busting parameter included — `{CDN_URL_PREFIX}/{key}?v={upload-unix-nanos}` — written once at upload and returned verbatim as `public_url`. This follows [lfx-v2-member-service](https://github.com/linuxfoundation/lfx-v2-member-service), the one V2 service that has shipped the pattern (`internal/service/logo_uploader.go`). It resolves a contradiction: the `?v=` token must be persisted rather than refetched per response, but `avatar_url`/`logo_url` are single `TEXT` columns with nowhere for a companion timestamp, and [03 §migration](./03-migration-plan.md#migration-specific-tasks) adds no schema change. Embedding the token in the stored URL satisfies both.

  **Three pieces carry this, and all three are required together:** (1) a **deterministic object key** — one per resource and file class (`program_logos/{program_uid}`, `profile_logos/{profile_uid}`, `avatars/{user_uid}`), reused by every upload, no UUID and no extension, with `Content-Type` on the object rather than inferred from the key; (2) the stored string is the versioned URL; (3) `Cache-Control: public, max-age=86400` as object metadata at upload. **The `?v=` token is Unix nanoseconds, not seconds** — at second resolution two uploads to the same key within one second mint the same token, and a CDN edge that cached the first serves it back for the second until the TTL expires. member-service enforces this deliberately (`internal/infrastructure/objectstore/client.go:263-268`, LFXV2-2016); `lfx-self-serve`'s avatar path uses seconds (`object-store.service.ts:298`), which is a known inconsistency, not a second precedent. **Determinism is the load-bearing part** — every upload overwrites the same key, so a URL persisted elsewhere (a stale index row, a cached response) points at a key whose *current* bytes are correct and converges within the TTL. A UUID-per-upload key would freeze every stale URL permanently. **Determinism makes every upload a blind overwrite of one object, so the replace needs a named serialization point.** Two concurrent uploads for the same resource otherwise race: both `PutObject` the shared key and both update the row, leaving the final bytes and the final stored URL chosen by two independent races. A conditional or row-locked Postgres update on the owning record is sufficient here — it must be the single point that decides the winner. member-service faced the same problem and names its Salesforce conditional update as that point, with `CopyIfNewer` plus a bounded arbiter loop on the object side (`internal/service/logo_uploader.go`); Mentorship does not need that arbitration protocol, but it does need *a* named point rather than none. **Do not lengthen the TTL without revisiting this**, and do not use the S3 `VersionId` as the token: it is an addressing mechanism, and code that passes it to `GetObject` pins stale URLs to frozen bytes. The `?v=` parameter must be in the CloudFront cache key.

  Two consequences. `CDN_URL_PREFIX` is baked into a data column, so changing the CDN hostname needs a rewrite pass over every public-class row — accepted as cheaper than a schema change, and bounded to a prefix substitution. And the service's own download route composes the key from the record's identity rather than parsing it back out of the stored URL, so it still works when `CDN_URL_PREFIX` is unset. `CopyObject` preserves source metadata by default, so the migration copy must pass `MetadataDirective: REPLACE` to set `ContentType` and `Cache-Control`.
- **Private files** store the **S3 object key** (for example `resumes/{user_uid}/{uuid}.pdf`), not a URL and not a route. **This class needs companion metadata the current columns cannot hold.** `02` requires serving private downloads with `Content-Disposition: attachment`, and the design skill lists filename, content type and size as required metadata — but `tasks.file` and `profile_links.resumeLink` are bare scalars, and keying private singletons deterministically (below) drops the extension too, removing the last in-band type hint. Recording the original filename, content type and size alongside the key is therefore a **schema change this design requires**, tracked as part of the file-routes implementation rather than assumed away; see [03 §migration-specific tasks](./03-migration-plan.md#migration-specific-tasks). The download route is derived from the owning entity, so a stored route would carry an entity ID and no key — leaving the handler nothing to pass `GetObject`. The bucket has no CDN attached.

  **The key is repository-internal and must never appear in a response body.** This is a requirement on the API, not a property it has today: `tasks.file` and `user_profiles.profile_links` are currently serialized straight to the client (`Task.File`, `UserProfile.ProfileLinks`), so adopting this mapping without also projecting those fields out would replace a public URL with an internal locator in the same response — strictly worse than what it replaces. Responses carry the **download route** for callers the ruleset admits. This applies to the private class only: `avatar_url` and `logo_url` hold a public CDN URL that is *meant* to be returned, and are unaffected. For `tasks.file` it means omitting the field entirely for callers the ruleset does not admit, but `user_profiles.profile_links` also carries `linkedinProfileLink` and `githubProfileLink` — public metadata unrelated to resume access — so denying resume access omits or replaces **`resumeLink` alone**, matching the per-field reservation in [06 §file routes](./06-route-matrix.md#file-routes); the other two links are always returned. Whether the projection is done with response DTOs or by narrowing the models is an implementation choice; the contract is only that the key does not leave the service.

Per-file cap is **20 MB** — the platform ceiling, not the per-class limit. **Image classes (program logos, profile logos, avatars) cap at 2 MB**, matching every shipped V2 image path: member-service's org logo (`pkg/constants/logo.go:13`) and `lfx-self-serve`'s avatar, which shipped at 20 MB and was rolled back to 2 MB (LFXV2-2628). A 20 MB PNG behind `max-age=86400` on an asset rendered in every list view is the cost that rollback was fixing. Server-side downscaling is **out of scope** for the rewrite — member-service downscales rasters above a dimension bound, and Mentorship may adopt that later, but the 2 MB cap is a reject, not a resize. No presigned uploads and no resumable/chunked uploads. Buckets are private with versioning, SSE, and lifecycle rules. Write access in **deployed** environments is via IRSA; the local `nats-s3` sidecar uses a locally generated static SigV4 pair, which is the platform's documented local mode and must never appear in a deployed values file.

### Deleting the bytes

**Every path that drops a file's locator must also remove the object. Bucket lifecycle is not a substitute.** The ops lifecycle rules expire *noncurrent* versions ([lfx-object-store-ops](https://github.com/linuxfoundation/lfx-skills/blob/main/skills/lfx-object-store-ops/SKILL.md)); a current object whose last database reference is gone is unreferenced, not noncurrent, and nothing reaps it — for a resume or submission, live PII with no expiry. Three paths need it, only the first covered by the file `DELETE` routes in [06](./06-route-matrix.md):

- **Replacement.** Public classes are safe by construction — the deterministic key overwrites in place, and the prior payload becomes a noncurrent version that lifecycle expires. Private classes are **not**: `resumes/{user_uid}/{uuid}.pdf` mints a new key per upload, so a `PUT` strands the old object. Prefer keying private singletons deterministically too, rather than a cleanup call that can fail after the row is already rewritten.
- **Parent deletion.** `DELETE /v1/programs/{uid}`, `DELETE /v1/tasks/{uid}`, `DELETE /v1/me`, and `DELETE /v1/me/profile` all remove rows that hold file locators — as do application and term deletes once `tasks.application_id` becomes `ON DELETE CASCADE` ([03 §migration-specific tasks](./03-migration-plan.md#migration-specific-tasks)), which removes task rows and their locators without touching those routes — `user_profiles.user_id` is `ON DELETE CASCADE`, so deleting a user erases `resumeLink` in Postgres without touching S3. Each must enumerate the file keys the deleted rows held and remove them.
- **A write that half-succeeds.** `PutObject` succeeds and the row write then fails. The object is a current, unreferenced one — precisely the case lifecycle never reaps — and no route ever learns of it. Either write the row first and treat a failed upload as a rolled-back transaction, or record the intended key before uploading so a failed write leaves something to clean up.
- **Permanently, on a versioned bucket.** For private classes, removal means deleting **all versions and delete markers**, not a plain `DeleteObject` — which only adds a marker and leaves every prior payload retrievable by version. This is the same operation [03 §migration](./03-migration-plan.md#migration-specific-tasks) already specifies for the destination reconcile — which runs at the *end* of the freeze rerun, after the final backfill, not at freeze time; it needs to hold at runtime too, not only at cutover. The ops IAM baseline grants neither `s3:DeleteObjectVersion` nor `s3:ListBucketVersions`, so the private-bucket role needs both added before this guarantee holds.

Deletion may lag the row delete, but must be retried to completion rather than dropped — a failure leaves PII behind with nothing pointing at it. **There is no scheduled sweep to fall back on:** the reconcile in [03 §migration-specific tasks](./03-migration-plan.md#migration-specific-tasks) is a one-time pre-cutover pass, and its manifest is destroyed once validation passes. A recurring unreferenced-object sweep would be the backstop for keys that escape the paths above, and is **not in scope here** — until one exists, each path above must clean up after itself.
...
**Every file class has a service download route, public ones included.** The CDN does not replace that route, it only supplements it for public reads: when `CDN_URL_PREFIX` is set, the API returns the CDN URL as `public_url` and clients fetch the bytes from the edge; when it is unset — local development without the `nginx-s3-gateway` stand-in — the service route is the only path. **Unsetting the prefix is a deploy-time choice, not an outage response:** rows written while it was set still hold CDN URLs, so flipping it does not redirect existing content. A CDN outage is handled at the edge (origin failover) or by a client retrying the service route explicitly; the route's value during one is that the bytes remain reachable at all, not that clients rediscover it automatically. The public download routes carry the same relation as the record that holds the URL **where the record has one to carry** — that is true of `GET /v1/programs/{uid}/logo` (`viewer`), but the profile and avatar reads are `allow_all` with a service-side publicly-listable check, since there is no profile object type in the model ([06 §file routes](./06-route-matrix.md#file-routes)). Either way the service route is not equivalent to the CDN path: the CDN serves the object to anyone holding the URL, with no relation check at all. The service route is the authorized path; the CDN is an unauthenticated cache in front of the same bytes, which is why only classes that are public by intent may be CDN-fronted (RM-6).
